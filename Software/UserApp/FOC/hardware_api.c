...
void _currentGetValue(float *cs_value){
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        cs_value[i] = Low_Pass_Filter(&current_1[i] , HAL_ADC_GetValue(&hadc1) - cs1_zero_value[i], 0.6f);
    }
...
void _currentGetValue2(float *cs_value){
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // 该函数读取寄存器DR同时自动清除了EOC(End Of unitary Conversation)标志位
        // cs_zero_value需要实际测量得出
        cs_value[i] = Low_Pass_Filter(&current_2[i] , HAL_ADC_GetValue(&hadc2) - cs2_zero_value[i], 0.6f);
    }
    // Stop ADC conversion of regular group, disable ADC peripheral.
    HAL_ADC_Stop(&hadc2);
}